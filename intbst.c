...
int main( int argc, char **argv)
{
	...
	while (1)
	{
		...
		int num;
		int ret = scanf( "%d", &num);
		if (ret != 1) break;//여기서 ret은 들어오는 숫자의 개수를 뜻하는건가?
...
int BST_Delete( TREE *pTree, int dltKey){//tree, num
	int *success;
	int num = 0;
	success = &num;
	...
	pTree->root = _delete(pTree->root, dltKey, &num);
	return num;
